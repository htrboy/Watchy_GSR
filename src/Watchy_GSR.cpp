...
RTC_DATA_ATTR char city;
RTC_DATA_ATTR int cityNameID = 0;
...
RTC_DATA_ATTR uint8_t weatherFormat = 1; // farenheit
RTC_DATA_ATTR uint8_t weatherMode = 0; // every half-hour
...
long connectMillis = 0; // for weather request
      ...
      if (ActiveMode == true) {
        ...
        while (ActiveMode == true) { // Here, we hijack the init and LOOP until the NTP is done, watching for the proper time when we *SHOULD* update the screen to keep time with everything.
          ...
          if ((Options.SleepStyle == 1 || Options.SleepStyle > 2 || WatchTime.DeadRTC) && !WatchTime.BedTime) {
            ...
            else if (Darkness.Tilt != 0 && millis() - Darkness.Tilt > 999 && Darkness.Woke) {
              Darkness.Last = millis();
            }
          }
...
void WatchyGSR::drawMenu() {
  ...
  if (Menu.Item == MENU_STEPS) { //Steps
    ...
    switch (Menu.SubItem) {
      ...
      case 1:
        if (TimerUp.Active) {
          y1 = ((WatchTime.UTC_RAW - (TimerUp.SetAt + WatchTime.UTC.Second)) / 3600);
          x1 = (((WatchTime.UTC_RAW - (TimerUp.SetAt + WatchTime.UTC.Second)) + 3600 * y1) / 60);
        }
        else {
          y1 = ((TimerUp.StopAt - TimerUp.SetAt) / 3600);
          x1 = (((TimerUp.StopAt - TimerUp.SetAt) + 3600 * y1) / 60);
        }
    ...
    switch (Menu.SubItem) {
      ...
      case 2:
        ...
        break;
      case 3:
        if (WatchTime.DeadRTC) O = "Bad RTC"; else {
          if (Options.UsingDrift) {
            if (Options.Drift < 0) O = "+" + String(0 - Options.Drift) + " " + MakeSeconds(0 - Options.Drift);
            else O = "-" + String(Options.Drift) + " " + MakeSeconds(Options.Drift);
          } else O = "No Drift";
        }
    }
...
void WatchyGSR::ProcessNTP() {
  ...
  switch (NTPData.State) {
    ...
    case 99: {
        ...
        if (NTPData.TimeTest && Menu.Item == MENU_TOFF) {
          Menu.SubItem = 2;
          NTPData.TestCount = 0;
        }
...
void WatchyGSR::handleButtonPress(uint8_t Pressed) {
  ...
  if (Darkness.Went) {
    Darkness.Woke = true;  // Don't do the button, just exit.
    Darkness.Last = millis();
          ...
          switch (Menu.SubItem) {
            ...
            case 3: // SleepStart (hour)
              if (Options.SleepStart + 1 != Options.SleepEnd) {
                Options.SleepStart = roller(Options.SleepStart + 1, 0, 23);
                Options.NeedsSaving = true;
              } else return;
              break;
...
float WatchyGSR::getBatteryVoltage() {
  return ((BatteryRead() - 0.0125) +  (BatteryRead() - 0.0125) + (BatteryRead() - 0.0125) + (BatteryRead() - 0.0125)) / 4;
}

float WatchyGSR::BatteryRead() {
  return analogReadMilliVolts(SRTC.getADCPin()) / 500.0f;  // Battery voltage goes through a 1/2 divider.
}
...
void WatchyGSR::rightJustify(const String txt, uint16_t &yPos) {
  int16_t x1, y1;
  uint16_t w, h;
...
// Override functions.
uint16_t WatchyGSR::ForeColor() {
  return (Options.LightMode ? GxEPD_BLACK : GxEPD_WHITE);
}

uint16_t WatchyGSR::BackColor() {
...
void WatchyGSR::InsertWiFi() {
  if (USEDEBUG) {
    Serial.println("--- InsertWiFi ---");
    Serial.println("getWeather.state: " + String(getWeather.state));
  }
  if (getWeather.state > 0 && getWeather.state != 99) {                                   // getWeather
    if (getWeather.pause == 0) processWxRequest(); else getWeather.pause--;               // getWeather
  }                                                                                       // getWeather   
}
...
String WatchyGSR::InsertNTPServer() {
  return "pool.ntp.org";
}

void WatchyGSR::AllowDefaultWatchStyles(bool Allow) {
  DefaultWatchStyles = Allow;
}

bool WatchyGSR::IsDark() {
...
String WatchyGSR::MakeSeconds(uint8_t Seconds) {
...
void WatchyGSR::AskForWiFi() {
  if (!GSRWiFi.Requested && !GSRWiFi.Working) GSRWiFi.Requested = true;
  ...
  if (USEDEBUG) {
    ...
    Serial.println("GSRWiFi.Index: " + String(GSRWiFi.Index));
...
void WatchyGSR::processWiFiRequest() {
  //Serial.println("---ProcessWiFiRequest---");
  ...
  if (GSRWiFi.Working) {
    if (getButtonPins() != 2) OTATimer = millis(); // Not pressing "BACK".
    if (millis() - OTATimer > 10000 || millis() - OTAFail > 600000) OTAEnd = true;  // Fail if holding back for 10 seconds OR 600 seconds has passed.
    if (rWiFi == WL_CONNECTED) {
      GSRWiFi.Working = false;  // We got connected.
    ...
    if (millis() > GSRWiFi.Last) {
      ...
      if (GSRWiFi.Index < 11) {
        if (GSRWiFi.Index == 0) {
          ...
          GSRWiFi.Tried = true;
          if (WiFi_DEF_SSID > "") { //use hardcoded AP/PW if set
...
void WatchyGSR::StoreSettings(String FromUser) {
  ...
  J = 0; if (L > J && O[J] > 128) {
    NewV = O[J];  // Detect NewVersion and go past.
    J++;
  }
  ...
  S = ""; for (I = 0; I < 10; I++) {
    strcpy(GSRWiFi.AP[I].APID, S.c_str());
    strcpy(GSRWiFi.AP[I].PASS, S.c_str());
  }
...
bool WatchyGSR::InTurbo() {
  return (!WatchTime.DeadRTC && Options.Turbo > 0 && TurboTime != 0 && millis() - TurboTime < (Options.Turbo * 1000));
}
  ...
  if (Options.SleepStyle > 0) {
    ...
    if (Options.SleepStyle == 2) {
      ...
      if (Options.SleepEnd > Options.SleepStart) {
        if (WatchTime.Local.Hour >= Options.SleepStart && WatchTime.Local.Hour < Options.SleepEnd) return false;
      }
...
uint8_t WatchyGSR::getTXOffset(wifi_power_t Current) {
  uint8_t I;
  for (I = 0; I < 11; I++) {
    if (RawWiFiTX[I] == Current) return I;
  }
  return 0;
}
...
bool WatchyGSR::SafeToDraw() {
  return (!(OTAUpdate || WatchyAPOn || (Menu.Item == MENU_TOFF && Menu.SubItem == 2)));
}
...
void WatchyGSR::moon_earth(SolarThings &solar, time_t &utc_time, uint16_t &time_zone) {
  ...
  if (moon_y > earth_y) {
    display.setTextColor(dColor);
    display.setFont(&Font12);
    display.setCursor(earth_x - 5 * 8 / 2, earth_y - 20);
    display.print("earth");
  } else {
    display.setTextColor(dColor);
    display.setFont(&Font12);
  ...
  display.setTextColor(dColor);
  display.setFont(&Font12);
  display.setCursor(moon_x - 50, moon_y + 8);
  display.print("moon");
}

float WatchyGSR::sun_paths(SolarThings &solar, time_t &utc_time, uint16_t &time_zone) {
  uint16_t dColor = Options.LightMode ? GxEPD_WHITE : GxEPD_BLACK;
  short WIDTH = 200; //280
  short HEIGHT = 55; // +20 ...portion of screen?
  short X = 0; //+20
  short Y = 200 - 25; //
  short N = 60;
...
//  display.drawLine(0, zero_deg, WIDTH, zero_deg, dColor);

  // draw sun rise/set sinewave
//  for (int i = 1; i <= N; i++) {
//    short x1 = ((i - 1) * (SEC_IN_DAY / N)) / ((double) SEC_IN_DAY) * WIDTH + X;
//    short x2 = (i * (SEC_IN_DAY / N)) / ((double) SEC_IN_DAY) * WIDTH + X;
//    short y1 = (1 - angles[i - 1]) * HEIGHT + Y; //change from + Y
//    short y2 = (1 - angles[i]) * HEIGHT + Y; //change from + Y
//
//    display.drawLine(x1, y1, x2, y2, dColor);
//  }

  // draw sun on sunrise/sunset arc
...
//  display.fillCircle(sun_x, (1 - current_angle) * HEIGHT + Y, 7, dColor); //

  //solar.sunrise(lon, eqTime, ha, timezone)
  long sunrise_time = solar.sunrise((utc_time_day_start + SEC_IN_DAY / 2), time_zone);
  short line_x = (sunrise_time % SEC_IN_DAY) / ((double) SEC_IN_DAY) * WIDTH + X;
//  // vertical line
//  display.drawLine(line_x, zero_deg, line_x, Y - 3, dColor);

  long sunset_time = solar.sunset((utc_time_day_start + SEC_IN_DAY / 2), time_zone);
  short line_x_2 = (sunset_time % SEC_IN_DAY) / ((double) SEC_IN_DAY) * WIDTH + X;
//  // vertical line
//  display.drawLine(line_x_2, zero_deg, line_x_2, Y - 3, dColor);

  String sunriseTime = String(String((sunrise_time / (60 * 60)) % 24) + ":" + String(sunrise_time / 60 % 60));
  display.setFont(&Font12);
  display.setTextColor(dColor);
  display.setCursor(10, Y); //line_x - 7 * 7 / 2
  display.print("sunrise");
  display.setCursor(10, Y + 15); //line_x - 7 * 5 / 2
  display.print(sunriseTime);

  String sunsetTime = String(String((sunset_time / (60 * 60)) % 24) + ":" + String(sunset_time / 60 % 60));
  display.setFont(&Font12);
  display.setTextColor(dColor);
  display.setCursor(145, Y); //line_x_2 - 7 * 6 / 2
  display.print("sunset");
  display.setCursor(145, Y + 15); //line_x_2 - 7 * 5 / 2
  display.print(sunsetTime);

  return current_angle;
}

int WatchyGSR::rtcTemp() {
  //  temperature = (RTC.temperature() / 4) - ambientOffset; //celsius
  //uint8_t _rtcTemp = rtc.temperature();
  temperature = SBMA.readTemperature() - ambientOffset; //celsius
  return temperature;
}
...
switch (getWeather.state) {
    ...
    // Am I Connected?  If so, ask for weather.
    // should only run if connected
    case 2: {
        if (WiFi.status() != WL_CONNECTED) {
          if (currentWiFi() == WL_CONNECT_FAILED) {
            getWeather.pause = 0;
...
weatherData WatchyGSR::askForWeather() {
  ...
  // String weatherQueryURL = String(URL) + String("?q=") + String("Portland") + String(",") + String(COUNTRY); // alternate Query

  String weatherQueryURL = String(URL) + String("?id=") + String("5746545"); // primary query for now

  weatherQueryURL = weatherQueryURL + String("&units=metric&appid=") + String(APIKEY);

  if (USEDEBUG) {
    Serial.println("URL: " + (weatherQueryURL));
  }

  // send the query to the website and parse response
  HTTP.begin(weatherQueryURL.c_str());
  int httpResponseCode = HTTP.GET(); //HTTP.GET
  ...
  if (httpResponseCode == 200) {
    ...
    if (USEDEBUG) {
      Serial.println("WX success");
    }
    
    getWeather.updateWx = false;
    getWeather.check = true; // success
    //getWeather.state = 99;
    //endWiFi();
    ...
    //http error
    getWeather.updateWx = false;
    getWeather.check = false; // weather check failed
    temperature = rtcTemp();
